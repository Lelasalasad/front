...
const Cross_wallet_exchange = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (parseFloat(amount) > balance) {
      toast.error(
        t("insufficientBalance", { wallet: accountNumber, currency })
      );
      return;
    }
    ...
    try {
      const res = await axios.post(
        "http://127.0.0.1:8000/api/crosswalletexchange",
        {
      ...
      if (res.data && res.data.message) {
        toast.success(
          t("transferSuccess", { amount, accountNumber, currency })
        );
        setAmount("");
        setAccountNumber("");
        setCurrency("");
      ...
      const msg =
        error.response?.data?.message || error.message || t("transferFailed");
      toast.error(msg);
    } finally {
      setLoading(false);
    }
