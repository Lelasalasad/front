...
const In_wallet_exchange = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!wallet || !wallet[fromCurrency] || parseFloat(amount) > wallet[fromCurrency]) {
        ...
        try {
            const res = await axios.post('http://127.0.0.1:8000/api/wallet/exchange', {
            ...
            if (res.data && res.data.converted_amount) {
                ...
                setErrorMessage('');
                toast.success(t('exchangeSuccess', {
    ...
    console.error('تفاصيل الخطأ:', error.response?.data?.errors); // لعرض الأخطاء الدقيقة

            const msg = error.response?.data?.message || error.message || t('exchangeFailed');
            setErrorMessage(msg);
            toast.error(msg);
        }
    };
